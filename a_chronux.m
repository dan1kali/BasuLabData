...
%   - Extracts power data, and generates features from that power data (not z scores)
...
function preProcess(patient)
    ...
    outputFolder = fullfile('outputDataChronux_zscore', outputName);
...
% {trials} [freq x time × Nchannels]
% Changed all nChannels, dimensions accordingly

    inputPath = fullfile('outputDataChronux_zscore', patient);
    outputName = patient;
    ...
    for ch = 1:nChannels
        ...
        for tr=1:nTrials                        
            ...
            % % band_power_norm{tr} (ch, :) = log(mean((S_power_final / S_baseline_meaned_rep),1));  % wrong
            ...
            band_power_norm{tr} (ch, :) = (S_power_norm - mu) / sigma;  % Store in 3D matrix
