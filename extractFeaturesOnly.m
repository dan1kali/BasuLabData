...
% 
%     % Get the patient code from the file name (e.g., "UC01.mat" -> "UC01")
%     [~, patientCode, ~] = fileparts(matFiles(i).name);
% 
%     % Create a folder for the patient (if it doesn't already exist)
...
% Changed all nChannels, dimensions accordingly
    ...
    inputPath = fullfile('patientPowerData', patient);
    outputName = patient;
    ...
    for tr=1:nTrials
        ...
        for ch=1:nChannels
            ...
            S_epoched = exp(PowerData(tr,ch,tWindow));

            % [1st column - mean, 2nd column - max]
            band_power_mean_max{tr}(:,1) = mean(S_epoched);  % mean over time
            band_power_mean_max{tr}(:,2) = max(S_epoched);   % max over time
    ...
    outputFolder = fullfile('outputPowerData_nolog','highGamma',outputName);
